...
public class DBHelper extends SQLiteOpenHelper {
	...
	public String[] getAllCityByProvinceID(int province_id) {

		String[] columns = { "name" };
		String[] arg = { province_id+""};
		SQLiteDatabase db = getReadableDatabase();
		// 查询获得游标
		Cursor cursor = db.query("citys", columns ,"province_id = ? ",arg, null, null,
				null);
		columns = null;
		int count = cursor.getCount();
		String[] citys = new String[count];
		count = 0;
		while (!cursor.isLast()) {
			cursor.moveToNext();
			citys[count] = cursor.getString(0);
			count = count + 1;
		}
		cursor.close();
		db.close();
